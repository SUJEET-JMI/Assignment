...
const downloadPDF = () => {
  ...
  const handleStatusChange = async (index, status) => {
    ...
    try {
      ...
      // If status changed to something other than SUSPENDED, remove from list
      if (status !== "SUSPENDED") {
        const updated = teachers.filter(s => s.teacherId !== teacher.teacherId);
        setteachers(updated);
      } else {
        // Update local state if still SUSPENDED
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <div
                      onClick={() => navigate(`/teachers/profile/${s.teacherId}`)}
