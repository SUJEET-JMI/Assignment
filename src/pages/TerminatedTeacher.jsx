...
export default function TerminatedTeacher() {
  const [TerminatedTeacher, setTerminatedTeacher] = useState(TerminatedTeacherData);
  const [openStatusIndex, setOpenStatusIndex] = useState(null);
  const [selectedTerminatedTeacher, setSelectedTerminatedTeacher] = useState([]);
  ...
  const filteredTerminatedTeacher = TerminatedTeacher.filter(student =>
    student.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const downloadExcel = () => {
    const ws = XLSX.utils.json_to_sheet(filteredTerminatedTeacher);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "TerminatedTeacher");
    XLSX.writeFile(wb, "TerminatedTeacher.xlsx");
  };
...
const downloadPDF = () => {
  ...
  // Title
  doc.setFontSize(16);
  doc.text("TerminatedTeacher List", 14, 15);
  ...
  // Table rows
  const rows = filteredTerminatedTeacher.map((student) => [
    student.id,
    student.name,
    student.email,
    student.mobile,
  ...
  // Download PDF
  doc.save("TerminatedTeacher.pdf");
};


  const handleStatusChange = (index, status) => {
    const updated = [...TerminatedTeacher];
    updated[index].status = status;
    setTerminatedTeacher(updated);
    setOpenStatusIndex(null);
  };

  const toggleSelectOne = (id) => {
    setSelectedTerminatedTeacher((prev) =>
      ...
      <div className="bg-white rounded-lg border">
        <div className="overflow-x-auto h-[70vh] overflow-y-auto relative">
          <table className="min-w-[1200px] text-sm">
            ...
            <tbody>
              {filteredTerminatedTeacher.map((s, i) => (
                <tr
                  key={s.id}
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <div
                      onClick={() => navigate(`/TerminatedTeacher/profile/${s.id}`)}
                  ...
                  <td className="px-6 py-5">
                    <input
                      type="checkbox"
                      checked={selectedTerminatedTeacher.includes(s.id)}
